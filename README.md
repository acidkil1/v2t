...
Займёт 2–5 минут в зависимости от скорости интернета.
  ...
  4. _(Опционально)_ **Включить GPU-ускорение.**  
...
Запусти `v2t.bat` без аргументов — покажет все доступные флаги и значения по умолчанию.

    v2t.bat "C:\path\to\video.mp4"
    v2t.bat "C:\path\to\video.mp4" --model medium --lang en
    v2t.bat "C:\path\to\video.mp4" --device cpu --keep-audio

Результат — `video_transcript.txt` рядом с видео (или в `--out`-папке). Промежуточный `.ogg` удаляется автоматически; чтобы сохранить — добавь `--keep-audio`.
...
`--keep-audio` | выкл | Сохранять промежуточный `.ogg` (по умолчанию удаляется)  
...
## Как это работает
  ...
  2. **faster-whisper** (реализация Whisper от OpenAI на CTranslate2) транскрибирует аудио. На GPU работает в 5–10× быстрее, чем оригинальный Whisper.
